  ...
  return <header className="wiki-header">
    <div className="header-main">
      ...
      <nav className="main-nav" aria-label="Main navigation">
        {[['weapons', copy.nav.weapons], ['weapons/grimoire', copy.nav.builds], ['artifacts', copy.nav.artifacts], ['bosses', copy.nav.bosses], ['coop', copy.nav.coop], ['guides', copy.nav.guides]].map(([path, label]) => <Link key={path} href={href(path)}>{label}</Link>)}
      </nav>
    ...
    <div className="header-utility"><nav className="utility-nav" aria-label="Utility navigation">
      {[['#updates', copy.utility.update], ['#roadmap', copy.utility.roadmap], ['#codes', copy.utility.codes], [officialLinks.community, copy.utility.community], ['#faq', copy.utility.faq]].map(([path, label]) => <a key={path} href={path.startsWith('#') ? path : path} target={path.startsWith('http') ? '_blank' : undefined} rel={path.startsWith('http') ? 'noreferrer' : undefined}>{label}</a>)}
    </nav></div>
  </header>
}
...
export function WikiFooter({ locale, copy }: { locale: Locale; copy: Copy }) {
  ...
  return <footer className="footer"><div className="footer-inner"><div className="footer-grid">
    ...
    <div><h3>{copy.footer.guides}</h3><ul><li><Link href={href('/weapons')}>{copy.nav.weapons}</Link></li><li><Link href={href('/weapons/grimoire')}>{copy.nav.builds}</Link></li><li><Link href={href('/')}>{copy.home.aboutCta}</Link></li></ul></div>
