...
export default async function HomePage({ locale = 'en' as Locale }: { locale?: Locale }) {
  ...
  return <WikiShell locale={locale} copy={copy}>
    ...
    {locale === 'en' && <section className="section"><SectionHeading>Search-led guides</SectionHeading><h2 className="section-title gradient-title">Answer the questions players are asking</h2><div className="card-grid"><Link className="wiki-card" href={href('/guides/sephiria-roadmap')}><div className="card-number">R</div><h3 className="card-title">Sephiria Roadmap</h3><p className="card-copy">Check the dated release and update record, then see which roadmap claims still need official confirmation.</p></Link><Link className="wiki-card" href={href('/guides/sephiria-how-many-chapters')}><div className="card-number">6</div><h3 className="card-title">How Many Chapters?</h3><p className="card-copy">Get the direct six-chapter answer and understand what the current research does not identify yet.</p></Link><Link className="wiki-card" href={href('/guides/sephiria-upgrade-tree')}><div className="card-number">U</div><h3 className="card-title">Upgrade Tree</h3><p className="card-copy">Explore the confirmed six branches and 200+ upgrades without an invented node list.</p></Link><Link className="wiki-card" href={href('/guides/sephiria-discord')}><div className="card-number">D</div><h3 className="card-title">Official Discord</h3><p className="card-copy">Use the stable Steam discussion entry for community updates and co-op leads.</p></Link></div></section>}
